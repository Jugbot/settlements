...
import io.github.jugbot.util.blockPredicate
...
case class has_nav_path_to(target: String) extends FaeBehavior:
  override def execute(actor: FaeEntity, state: Blackboard): BehaviorStatus =
    ...
    (state.get(target), currentTarget) match {
      case (Some(posA: BlockPos), Some(posB: BlockPos)) if posA.distManhattan(posB) <= FaeEntity.NAVIGATION_PROXIMITY =>
        BehaviorSuccess
      case _ => BehaviorFailure
    }
...
case class target_closest_block(block: String) extends FaeBehavior:
  override def execute(actor: FaeEntity, state: Blackboard): BehaviorStatus =
    if !actor.isRandomTicking then return BehaviorRunning
